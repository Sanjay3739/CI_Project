...
class LandingPageController extends Controller
{
    ...
    public function index()
    {
        ...
        $users = User::where('user_id', '!=', Auth::user()->user_id)
            ->orderBy('user_id', 'asc')
            ->get();
        // $rating_a = MissionRating::where('mission_id')
        //     ->get()
        //     ->pluck('rating');
        // $rating = 0;
    ...
    public function filterApply(Request $request)
    {
        if ($request->ajax()) {
            ...
            $users = User::where('user_id', '!=', Auth::user()->user_id)
                ->orderBy('user_id', 'asc')
                ->get();
            return view('home.gridListView', compact('count', 'data', 'favorite', 'users', 'user_id'));
        }
    }
    ...
    public function findCity(Request $request)
    {
        if ($request->ajax()) {
            ...
            $city_ids = array_unique($citys);
            $cities = City::whereIn('city_id', $city_ids)->get(['city_id', 'name']);
            return view('home.city-dropper', compact('cities'));
        }
    }
    ...
    public function findTheme(Request $request)
    {
        if ($request->ajax()) {
            ...
            return view('home.theme-dropper', compact('themes'));
        }
    }
    ...
    public function findSkill(Request $request)
    {
        if ($request->ajax()) {
            ...
            // return view('home.skill-dropper', compact('skills'));
        }
    }
} { {
    }
