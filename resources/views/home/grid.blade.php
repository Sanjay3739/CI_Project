        ...
        <div class="d-flex justify-content-center py-1">
            ...
            <div class="position-relative" style="width: 415px; height: 247px;">
                ...
                <div class="position-absolute current-status">
                    ...
                    @elseif ($item->missionApplication->where('user_id', $user_id)->first()->approval_status ==
                    'DECLINE')
                ...
                <div class="position-absolute parent_like_btn">
                    <button id="mission_like_btn_{{ $item->mission_id }}_{{ $user_id }}" type="button"
                        class="like_btn py-1">
                        <?php $set = false;
                                $value = '0'; ?>
                ...
                <div class="position-absolute parent_add_btn">
                    <button class="add_btn py-1" id="misison_invite_btn_{{ $item->mission_id }}_{{ $user_id }}"
                        data-toggle="modal" data-target="#invite_user_modal_{{ $item->mission_id }}_{{ $user_id }}"><img
                            src={{ asset('Images/user.png') }} alt=""></button>
                    <!-- Modal -->
                    <div class="modal fade w-100" id="invite_user_modal_{{ $item->mission_id }}_{{ $user_id }}"
    ...
    <div class="card-body">
                ...
                <div class="d-flex py-3 justify-content-between">
                    ...
                    <div class='col-md-6 d-flex align-items-center justify-content-start'>
                        ...
                        <div class="d-flex flex-column w-100">
                            @if($achieved == $item->goalMission->goal_value ?? '45')
                            <marquee class="text-success " style="font-weight: 700">mission complated</marquee>
                            @endif
                            <div class="progress w-100">

                                <div class="progress-bar" aria-label="goal-reached" name="goal_status"
                                    style="width: {{($achieved/$item->goalMission->goal_value)*100}}%"
                                    role="progressbar" aria-valuenow="{{$achieved}}" aria-valuemin="0"
                                    aria-valuemax="{{$item->goalMission->goal_value}}"></div>
                            </div>
        ...
        <div class="d-flex justify-content-center">
            ...
            <button type="button" id="mission_application_btn_{{ $item->mission_id }}"
                data-mission_id="{{ $item->mission_id }}" data-user_id="{{ $user_id }}"
                class="btn btn-lg fs-6 apply-btn"> Apply <i class="fa-sharp fa-solid fa-arrow-right"></i>
            </button>
            @else
            <a href="{{ route('mission-page', $item->mission_id) }}"><button
                    id="mission_detail_btn_{{ $item->mission_id }}" class="mx-2 btn btn-outline apply-btn fs-6 px-2">
                    View Details <i class=" fa-sharp fa-solid fa-arrow-right"></i>
                </button></a>
            @endif
            <a href="{{ route('mission-page', $item->mission_id) }}">
                <button style="display: none;" id="mission_detail_btn_{{ $item->mission_id }}"
                    class="mx-2 fs-6 btn btn-outline apply-btn px-2" style="width: fit-content"> View Details
